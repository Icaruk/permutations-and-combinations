...
<div class="results-section">
	...
	{#if typeof totalPermutations === "number" && totalPermutations > MAX_DISPLAY_LIMIT}
		<p class="warning">
			Hay demasiadas permutaciones para mostrar (más de {MAX_DISPLAY_LIMIT.toLocaleString("es-ES")}).
			{#if totalPermutations >= MAX_SAFE_INTEGER}
				El número exacto excede el máximo valor seguro en JavaScript ({MAX_SAFE_INTEGER.toLocaleString("es-ES")}).
			{/if}
		...
		{#if permutations.length <= CHECKBOX_LIMIT}
			<div class="actions">
				<button
					...
					on:click={() => toggleSelectAll(true)}>Seleccionar Todas</button
				>
				<button
					class="secondary-button"
					on:click={() => toggleSelectAll(false)}>Deseleccionar Todas</button
				>
				<button
					class="primary-button"
					on:click={copySelected}>Copiar Seleccionadas</button
				>
			</div>
